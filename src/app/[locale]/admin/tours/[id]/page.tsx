...
export default function EditTourPage({ params }: PageProps) {
    ...
    useEffect(() => {
        const fetchTourData = async () => {
            try {
                setLoading(true);
                const [tour, dests] = await Promise.all([
                ...
                if (tour) {
                    setFormData({
                        ...
                        itinerary: tour.itinerary && tour.itinerary.length > 0 ? tour.itinerary : [{ day: 1, title: '', activities: [''] }]
                    });
                }
            } catch (err: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await tourService.updateTour(id, tourUpdate as any);
            setSuccess(true);
            router.refresh();
            setTimeout(() => router.push('/admin/tours'), 2000);
        } catch (err: any) {
            console.error('Error updating tour:', err);
            setError(err.message || 'Failed to update safari package.');
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
                ...
                <p className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Unlocking blueprints...</p>
            </div>
        );
    }
    ...
    if (success) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6">
                ...
                <h1 className="text-4xl font-black text-[#0b1315] uppercase tracking-tighter">Safari <br /><span className="text-emerald-500 italic lowercase tracking-tight">updated!</span></h1>
                <p className="text-gray-400 font-black uppercase text-[10px] tracking-widest leading-none">The path has been redefined...</p>
            </div>
        );
    }
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-gray-100">
                <div className="space-y-4">
                    <Link href="/admin/tours" className="flex items-center gap-2 text-gray-400 hover:text-emerald-500 transition-colors group">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest leading-none">Cancel Changes</span>
                    </Link>
                    <div>
                        <h4 className="text-emerald-500 font-black uppercase text-[10px] tracking-[0.4em] mb-4">Refining Adventure</h4>
                        <h1 className="text-[#0b1315] font-black text-4xl md:text-5xl uppercase tracking-tighter leading-none">
                            Edit <br />
                            <span className="text-emerald-500 italic lowercase tracking-tight">safari</span>
                        </h1>
                    </div>
                </div>
                <button
                ...
                <div className="lg:col-span-2 space-y-12">
                    ...
                    <section className="space-y-8 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] block ml-2">Max Persons</label>
                                <div className="relative">
                    ...
                    <section className="space-y-8 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between">
                            <h3 className="text-[#0b1315] font-black uppercase text-xl tracking-tighter flex items-center gap-3">
                                <Calendar size={20} className="text-emerald-500" /> Experience Itinerary
                            </h3>
                            <button type="button" onClick={addItineraryDay} className="text-emerald-500 font-black uppercase text-[10px] tracking-widest hover:underline flex items-center gap-2">
                                ...
                                <div key={dIdx} className="p-8 bg-[#f8f9fa] rounded-[2.5rem] border border-gray-100 space-y-6 relative overflow-hidden group">
                                    ...
                                    <div className="flex items-center justify-between gap-4 relative z-10">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center text-white font-black text-xs">{day.day}</div>
                                            <input
                                                value={day.title}
                                                onChange={(e) => handleItineraryChange(dIdx, 'title', e.target.value)}
                                                className="bg-white border border-gray-200 rounded-xl py-2 px-4 text-[#0b1315] font-black uppercase text-sm tracking-tight focus:ring-1 focus:ring-emerald-500 w-full"
                                            />
                                        </div>
                                        <button
                                    ...
                                    <div className="space-y-4 relative z-10">
                                        {day.activities.map((act, aIdx) => (
                                            <div key={aIdx} className="flex gap-4 items-center">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-white border border-transparent rounded-xl py-3 px-4 text-gray-600 font-medium text-xs focus:ring-1 focus:ring-emerald-500 transition-all shadow-sm"
                                                />
                                                <button
                                        ...
                                        <button type="button" onClick={() => addActivity(dIdx)} className="text-gray-400 font-black uppercase text-[8px] tracking-[0.3em] hover:text-emerald-500 ml-6 flex items-center gap-2">
                ...
                <div className="space-y-12">
                    ...
                    <section className="space-y-8 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between">
                            ...
                            <button type="button" onClick={() => addListItem('highlights')} className="text-emerald-500 p-2 hover:bg-emerald-50 rounded-xl transition-colors">
                                <Plus size={18} />
                            </button>
                        </div>
                        <div className="space-y-4">
                            {formData.highlights.map((h, idx) => (
                                <div key={idx} className="flex gap-2">
                                    ...
                                    />
                                    <button onClick={() => removeListItem('highlights', idx)} className="text-gray-300 hover:text-red-500 transition-colors p-2">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            ))}
